        ...
        datasets: [
          {
            //label:'dolar',
            label:this.moneda,
            data: this.data.results.map((x:any) => x.detalle[0].tipoCotizacion
          ),
            tension: 0.3
          }
        ]
  ...
  this.httpClientService.getDivisa()
    .subscribe({
      ...
      next: (res) => {
        ...
        this.divisas = res.results.map((item: any): IDivisa => ({
...
filterCurrencies(event: any) {
  ...
  this.filteredDivisas = this.divisas.filter(divisa =>
